...
variable "tfstate_bucket" {
  ...
  type        = string
  default     = "tfo-tf-state-{AWS_ACCOUNT_ID}-{AWS_DEFAULT_REGION}"
}

variable "tfstate_dynamodb_table" {
  description = "Name of dynamodb table storing tfstate lock (from _tfstate stack)"
  type        = string
  default     = "tfo-ddb-tf-state-{AWS_ACCOUNT_ID}-{AWS_DEFAULT_REGION}"
}

variable "tfstate_path" {
  description = "Path .tfstate in Bucket (from _tfstate stack)"
  type        = string
  default     = "telemetryflow/{AWS_ACCOUNT_ID}/tfstate/terraform.tfstate"
}
...
variable "ami_os" {
  description = "OS for the AMI: 'debian' (13 Trixie, latest stable), 'ubuntu' (24.04 LTS) or 'amazon_linux_2023'"
  type        = string
  default     = "debian"
}
