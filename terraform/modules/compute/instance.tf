...
resource "aws_instance" "main" {
  ...
  # Per-instance user_data override: the launch template carries a generic
  # HOSTNAME (shared by every node) — unique hostnames are required because
  # RKE2 derives the Kubernetes node name from the system hostname.
  user_data = base64encode(
    templatefile(
      "${path.module}/template/user_data.sh",
      {
  ...
  # IMDSv2 with hop limit 2 — required by the EBS CSI driver pods to reach
  # the instance metadata service from inside Kubernetes.
  metadata_options {
    http_tokens                 = "required"
