...
if [[ "$OS" == "Debian" ]]; then
    # Debian: awscli from apt, cloudwatch agent from official .deb
    eval $PKG_INSTALL awscli curl htop jq netcat-openbsd net-tools wget
    ...
    eval $PKG_INSTALL /tmp/amazon-cloudwatch-agent.deb
else
    eval $PKG_INSTALL \
