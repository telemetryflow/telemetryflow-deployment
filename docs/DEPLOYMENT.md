...
## Stage-Runner Scripts (recommended)

Both ansible stacks are driven by stage-runner scripts — each playbook runs as
its own invocation, dry-run by default, with per-run logs and resumable stages.

```bash
# RKE2 cluster + TelemetryFlow (stages 00–04)
scripts/deploy-ansible-k8s.sh --list                # what would run
scripts/deploy-ansible-k8s.sh                       # dry-run (--check --diff)
scripts/deploy-ansible-k8s.sh --apply               # real run (asks to confirm)
scripts/deploy-ansible-k8s.sh --only 03 --apply     # redeploy helm chart only
scripts/deploy-ansible-k8s.sh --from 02 --apply \
  --values-file helm/values/staging/tfo-staging.yaml

# VM / Docker-Compose stack (ping → docker → db → platform → agent)
scripts/deploy-ansible.sh --list
scripts/deploy-ansible.sh --apply
scripts/deploy-ansible.sh --only 20,21 --apply      # databases only

# Generate platform secrets (dry-run prints; --apply fills the
# REPLACE_ME placeholders in helm/values/production/secrets/)
scripts/generate-secrets.sh
scripts/generate-secrets.sh --apply

# Platform + collectors (helm charts)
scripts/deploy-staging.sh                    # platform: helm/values/staging/tfo-staging.yaml
scripts/deploy-production.sh                 # platform: helm/values/production/tfo-production.yaml
scripts/deploy-tfo-collectors.sh --env staging --list

# Agent on a MONITORED SOURCE CLUSTER (helm/tfo-agent chart)
scripts/deploy-tfo-agent.sh --list
scripts/deploy-tfo-agent.sh --cluster telemetryflow --env staging
```

Two charts under `helm/`:

- `helm/telemetryflow` — the full platform stack (also carries the in-cluster
  agent + standalone collector releases)
- `helm/tfo-agent` — agent-only chart for source clusters (DaemonSet +
  tfo-agent-k8s Deployment + agent RBAC/secrets)

Logs land in `ansible{,-k8s}/artifacts/deploy/<run-id>/` (summary + per-stage
console captures). On failure the runner stops, points at the stage log, and
