...
| Variable                | Default                     | Description                                  |
| ----------------------- | --------------------------- | -------------------------------------------- |
| `rke2_version`          | `v1.36.4+rke2r1`           | RKE2 version to install                      |
...
| `telemetryflow_env`         | `demo`                                        | Selects deploy script + overlay (`demo` / `staging` / `production`) |
| `telemetryflow_chart_path`  | `{{ playbook_dir }}/../../helm/telemetryflow` | Local umbrella chart (no remote repo)     |
| `telemetryflow_uninstall_first` | `true` on demo                            | Uninstall the previous release before deploying (clean demo runs) |
